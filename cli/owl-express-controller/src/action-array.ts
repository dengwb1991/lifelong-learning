...
type IHandler<T extends any[]> = Route.FilterBeforeHandler<T> | Route.FilterAfterHandler<T>

/**
 * action 队列父类
 */
export class ActionArray extends Array<IHandler<any[]>> {
  public pushAction(handlerAction: any) {
 ...
 * @param context 上下文
 */
export const execute = (actions: any[], context: ControllerContext, data?: any, err?: Error) => {
  if (!actions.length) return context
  // @TODO 这里是否要注入回调函数？动态判断 injects 是否是 array
  return dispatch(0, data, err)

  function dispatch(i: number, prevRet?: any, err?: Error): Promise<any> {
