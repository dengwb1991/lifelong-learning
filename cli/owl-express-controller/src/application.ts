...
import { createController, createPath } from './controller'
import { readDirFile, createDebug } from './utils'
import { Route } from './route'
import { EventEmitter } from 'events'
import { Router as ExpressRouter, Express } from 'express'
import { Route as RouteInterface, ControllersOptions } from '~/.'
import { ActionArray } from './action-array'
const debug = createDebug('application')

type ExpressApp = Express | ExpressRouter

export class Application extends EventEmitter {
  public static DEFAULTS = {
    base: '/',
    matchFilename(filename: string) {
      if (!filename.match(/-controller\.[^\.]+$/i)) return false
   ...
   * 配置
   */
  public options: ControllersOptions
  /**
   * 路由
   */
  public routes: Route[]
  ...
  constructor(options: ControllersOptions) {
    super()
   ...
   * 挂载全局前置钩子函数
   */
  public before<T extends any[]= any[]>(action: RouteInterface.FilterBeforeHandler<T>) {
  ...
  // @TODO 应该还可以挂载全局的配置函数？
  public after<T extends any[]>(action: RouteInterface.FilterAfterHandler<T>) {
  ...
  private createRouter() {
    ...
    this.routes = routes.reduce((previous: Route[], fileInfo): Route[] => {
      ...
      if (typeof controller.default === 'function') {
        ...
        // 配置了驼峰转 path
        let uri = createPath(this.options.pathMode, controllerName)
        /**
         * fileInfo.uri === example/deep
         * 如果是 1 级分目录，则 fileInfo.uri 为 ""
         */
        if (fileInfo.uri) uri = `${fileInfo.uri}/${uri}`
        return previous.concat(createController(controller.default, controllerName, uri, this))
      } else {
        throw new Error(
  ...
  private mount() {
    this.routes.forEach(route => {
      const app = this.options.app as any
      if (!app[route.method]) {
        throw new TypeError(
          `[Controller.router:method]Property method does not support this value:${route.method}, route: ${route.path}`,
        )
      }
